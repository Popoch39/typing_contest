...
const TestGame = () => {
  ...
  const updateCursorPosition = () => {
    if (textRef.current) {
      ...
      const currentSpan = Array.from(spans).find(
        (span) => span.getAttribute("data-index") === cursorPosition.toString(),
      );

      console.log("new span", currentSpan);
      if (currentSpan) {
        ...
        const previousSpan = Array.from(spans)
          .reverse()
          .find(span => {
    ...
    handleClickRef.current = (letter: string) => {
      console.log(letter);
    console.log("boundaries", wordBoundaries[currentWordIndex]);
      if (letter === "Backspace") {
        ...
        if (extras.length > 0) {
          ...
          // check if error on delete, if its the case remove that err;
          if (mistakes.has(cursorPosition - 1)) {
            mistakes.delete(cursorPosition - 1);
          }
          console.log("case 3");
        ...
        if (cursorPosition >= currentWordBoundary.end) {
          ...
          if (letter !== text[cursorPosition]) {
            console.log("current pos", cursorPosition);
            console.log("case 8");
            console.log("mistake");
      ...
      return (
        <span key={wordIndex} className="relative">
          {word.split("").map((char, charIndex) => {
            ...
            let className = "transition-colors duration-500";
    ...
    return (
      <div className="p-8">
        ...
        `}</style>
          <p
            ref={textRef}
            className="text-2xl font-mono mb-4 leading-loose relative"
          >
            <span
          ...
          <div className="mt-4 text-gray-600 dark:text-gray-300">
            <p>
              Position : {cursorPosition} / {text.length}
            </p>
            <p>
              Mot actuel : {currentWordIndex + 1} / {wordsCount}
            </p>
